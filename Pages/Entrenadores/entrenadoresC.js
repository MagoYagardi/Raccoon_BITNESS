...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Función para conectar con el entrenador al hacer clic en el botón
    function connectWithTrainer(id_usuario) {
        // Hacer una solicitud al servidor para obtener los datos del entrenador
        fetch(`entrenadorModelo.php?id_usuario=${id_usuario}`)
            .then(response => response.json())
            .then(data => {
                if (data && data.length > 0) {
                    const telefono = data[0].telefono; // Asumimos que el teléfono está en el primer elemento de la respuesta
                    const mensaje = "¡Hola! Estoy interesado en tus servicios de entrenamiento.";
            ...
            .catch(error => {
                ...
                alert('Hubo un error al conectar con el entrenador');
            });
    }
